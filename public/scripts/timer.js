...
function updateClock(tc) {
    if (Number.isNaN(tc)) {
        ...
        if (i != this_phase_id) {
            document.getElementById("phaselist").children[i].classList.add("this_phase")
            document.getElementById("phaselist").children[this_phase_id].classList.remove("this_phase")
            this_phase_id = i;
        }
